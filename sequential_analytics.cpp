              ...
              const vector<pair<string, double>> &rows)
{
    ...
    f << "Task,Time_ms\n";
    for (auto &r : rows)
        f << r.first << "," << fixed << setprecision(3) << r.second << "\n";
...
int main(int argc, char *argv[])
{
    ...
    cout << "========================================\n";
    cout << " Sequential Data Analytics Pipeline\n";
    cout << " Dataset size: " << N << " points\n";
    cout << "========================================\n\n";
    ...
    vector<double> dataA(N), dataB(N);
    for (long long i = 0; i < N; i++)
    {
        dataA[i] = dist(rng);
        dataB[i] = dist(rng);
    }
    double genTime = t.elapsed_ms();
    cout << "[GEN]  Data generation       : " << fixed << setprecision(1) << genTime << " ms\n\n";
    ...
    cout << "       Mean=" << s.mean << "  StdDev=" << s.stddev
         << "  Min=" << s.minVal << "  Max=" << s.maxVal << "\n\n";
    ...
    // Save CSV
    string csvFile = "sequential_results_" + to_string(N) + ".csv";
